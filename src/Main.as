...
namespace TmMcpPackEpp {
    ...
    void RegisterPack() {
        ...
        Add(b, "SaveMacroblockFile", "Save a native .Macroblock.Gbx (real game file; auto-appears in the macroblock inventory). Sources: recorder stop-transfer (recorder/copyPaste) or tagged live placements (tagged + tag). Drives the engine's own save UI flow (snap scene + save-as dialog), so the file lands where the game puts saved macroblocks (Documents/Trackmania/Blocks/<Collection>/). If it fails with 'snap-camera scene did not open', the copy-paste selection is stale/consumed — record + stop a fresh selection and retry.", '{"type":"object","properties":{"name":{"type":"string"},"source":{"type":"string","enum":["recorder","copyPaste","tagged"]},"tag":{"type":"string"}},"required":["name"],"additionalProperties":false}');
